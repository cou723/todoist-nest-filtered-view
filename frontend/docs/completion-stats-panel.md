...
- 除外ラベル: `@毎日のタスク` は常時除外し、localStorage に保存されている追加除外ラベル（先頭の `@` は除去して比較）を適用する。除外ラベル編集 UI は未実装。
- 期間: 過去90日（当日を含めない履歴）+ 当日。履歴の移動平均計算のために先頭6日を余分に取得する。

## ユースケース
- `loadCompletionStatsConfig`: localStorage から除外ラベル設定を取得（未設定なら空配列）。
- `updateCompletionStatsConfig`: 除外ラベル設定を保存（現状 UI とは未接続、保存済み値を読む運用）。
...
## リポジトリ/ポート
- `CompletionStatsRepository`: TypeScript SDK (`getCompletedTasksByCompletionDate`) で完了タスクをページング取得（~30日ごとに分割取得し、ID で重複排除）。`CompletedTask` をドメイン型へ正規化する。
...
- hook `useCompletionStatsPanel` 内で config をロードし、依存に応じて統計/残件を自動取得（手動リフレッシュ・ポーリングなし、除外ラベルの編集 UI は未提供）。
- 表示内容: タイトル「ワークタスク完了統計」、サマリ（90日合計・残り@task数）、日次件数 + 7日移動平均のラインチャート（nivo）。
