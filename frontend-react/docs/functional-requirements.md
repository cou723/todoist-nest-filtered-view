...
- FE-F-021: `TodoistService.fetchTodosByFilter` は Todoist API のタスク取得エンドポイント（`GET https://api.todoist.com/rest/v2/tasks`）をカーソル付きでページングし、該当するタスクをすべて取得してからクライアント側でツリー構造（親タスクの再帰取得）を構築する。
...
- FE-F-050: 作業完了統計パネルは、Todoist API の完了タスクエンドポイント（`GET https://api.todoist.com/api/v1/tasks/completed/by_completion_date`）を用いて、作業タスクの完了履歴を取得する。
