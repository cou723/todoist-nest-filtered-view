# Todoist タスクリスト（Lit Legacy）

ユーザーの特定の条件に沿ったTodoistのタスクを取得し、そのタスクの親タスクの名前を含めて表示するクライアントアプリケーションです。本ディレクトリ `frontend-lit-legacy/` は現行運用中の Lit 実装で、React リプレイス版は `frontend-react/` に配置されています。新機能開発は React 版で行い、こちらはバグ修正と運用フォローのみを想定します。
